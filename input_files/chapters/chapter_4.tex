...
\begin{figure}
   \centering
   \subfigure[\label{fig-chap4-HR-crist-a}]%
        {\includegraphics[height=0.4\textwidth]{img/chapitre1/figure4/haadf-HR-sc.png}}
    \subfigure[\label{fig-chap4-HR-crist-b}]%
        {\includegraphics[height=0.4\textwidth]{img/chapitre1/figure7/drift-sc.png}}
   %
...
Toutes les expériences discutées dans la section suivante~\ref{sec-hr-results} sont réalisées sur un CPU Intel Xeon E5540 @ \np[GHz]{2.53} avec 8 c\oe{}urs (en incluant l'hyperthreading) et sur une mémoire de \np[Gb]{50}. \`A noter que seul BPFA requiert une telle mémoire, les autres méthodes peuvent fonctionner avec seulement \np[Gb]{13.2} de mémoire.
...
Les cartes d'erreur en NMSE sont représentées à la figure~\ref{fig-results-HR2_synth-bands}. Cette représentation peut être utile pour localiser les principales erreurs de reconstruction. De plus, la figure~\ref{fig-qu1-21-histograms} montre les histogrammes des erreurs pour chaque seuil. Ils montrent que l'erreur de PPV est généralement supérieure par rapport aux autres méthodes, tout particulièrement pour certains pixels.
...
\begin{normalfigure*}[htbp]
    ...
    %
    \caption{Spectres reconstruits pour $\mathsf{R}_2^*$ et pour un pixel non échantillonné dont la position dans l'image est donnée à la figure~\protect\ref{fig-synth-image-spectra-locations}. Les spectres de référence correspondent à l'image non bruitée $\bar{\mathsf{R}}_2$. Le zoom représente la région d'intérêt mise en évidence par la région transparente bleue. Les spectres de PPV et 3S sont significativement décalés par rapport à la référence tandis que les spectres reconstruits par CLS et les méthodes par \gls{ad} sont proches du spectre de référence.
